...
// Use regex to match the button by its onClick handler
data = data.replace(
  /\{isHost && \(\s*<button onClick=\{\(\) => socket\.emit\('host_back_to_lobby', \{ code \}\)\} className="btn-clean font-kufi bg-white text-black px-10 py-4 rounded-2xl text-xl font-bold shadow-\[0_0_20px_rgba\(255,255,255,0\.2\)\]\">\s*[\s\S]*?\s*<\/button>\s*\)\}/g,
...
data = data.replace(
  /\{isHost && \(\s*<button onClick=\{\(\) => socket\.emit\('host_back_to_lobby', \{ code \}\)\} className="btn-clean font-kufi bg-white text-black px-10 py-4 rounded-2xl text-xl font-bold mt-4 shadow-\[0_0_20px_rgba\(255,255,255,0\.2\)\]\">\s*[\s\S]*?\s*<\/button>\s*\)\}/g,
